    ...
    try:
        ...
        with conn.session as session:
            session.execute(
                ...
                """),
                {
                    "formal_name": formal_name, "handle": handle, "total_pieces": int(total_pieces), 
                    "total_cost": float(total_cost), "courier": float(courier), "amount_paid": float(amount_paid), 
                    "payment_status": payment_status, "line_items": line_items_json
                }
            )
            session.commit()
        return True
